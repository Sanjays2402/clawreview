...
- **CLI `presets diff --since-range <a>..<b>` with HEAD shorthand** — `--since-range main..` should resolve the empty target side to HEAD instead of rejecting (matches `git log a..` semantics). Today it rejects with "target ref is empty" -- the existing rejection has a real test (a comma-split typo would silently match the same case), so the design needs care.
- **Server `/api/reviews/:id/digest` `?slim` minus prefix** — `?slim=-byTag,-byFile` to denote "strip everything EXCEPT these" -- mirror of the exclude-fields surface on `presets diff`. Today it's an allowlist; the deny-list variant is the natural complement.
- **Telemetry `clawreview_review_drift_watch_polls_total` Prometheus exposition through the server** — the counter exists per-CLI-process; a CI runner using --watch can't easily scrape it. A small server endpoint that accepts batched CLI counter pushes (or a pull-side polling agent) would give an operator a single dashboard view of watch-loop health across every CI runner.
- **CLI `presets diff --since-range` JSON echoes `range` discriminator** — currently the JSON echoes `sinceRange: 'main..feature'` (the raw string). A consumer parsing the output has to re-derive whether it's two-dot or triple-dot; adding `sinceRangeKind: 'two-dot'|'triple-dot'` would mirror the existing internal discriminator and save a regex.
