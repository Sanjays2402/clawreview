...
- **Worker `findingDigest({ blame: ... })` server-side wiring** — carried.
- **Telemetry `clawreview_review_drift_watch_polls_total` Prometheus exposition through the server** — carried.
- **Aggregator `findingDigestFilterReport` helper for `?slim=true` projection** — when ?slim strips byTag/byCategory/byAgent/byFile on the digest, the persisted filterReport on the DTO should ALSO get a slim projection so a dashboard rendering "drop count only" doesn't need the full appliedFilters object. Default OFF; opt-in via `?slim=filterReport` (the existing slim field list).
- **CLI `review drift --base --target --on-regression <cmd>` hook** — when target has MORE findings than base in a bucket (regression detected), exec the hook with a JSONL payload of the regressed buckets. Mirror of the --watch --on-drift pattern but scoped to compare mode. Use case: CI pipeline that auto-files a JIRA ticket when a bug fix accidentally added new findings in another file.
- **CLI `stats --filter-summary --json-header --jsonl` mode** — when both --json-header and the new --jsonl flag are set, emit the header line + EACH severity bucket on its own line so a downstream tool can process the report incrementally. Mirror of the tick-15 review drift --watch JSONL output for the stats surface.
- **Server `/api/reviews/:id/filter-report` standalone endpoint** — pair the tick-22 DTO surface with a dedicated endpoint that returns ONLY the persisted filter report. Lighter wire payload than /api/reviews/:id (no findings array; no agentExecutions). Use case: a dashboard banner that only needs the drop count + applied filter shape doesn't have to pull the whole review.
- **Aggregator `findingDigestWithFilterReport` for tick-21's `?normalisedEcho` consumer flag** — when a CI consumer passes `?normalisedEcho=true` on the /digest endpoint, the persisted shape on rec.filterReport.appliedFilters carries the worker's normalised values verbatim. A dashboard could pull the persisted report and render "showing findings with confidence >= 1 (worker clamped from 1.5)" without re-running the digest normaliser. Today the dashboard re-derives via the same normaliser; the persisted slice already has the answer.
