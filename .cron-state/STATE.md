...
- **Findings page: virtualized list for 200+ findings** — carried (tick 30+). Windowed scroller (custom IntersectionObserver, no react-window dep) when `filtered.length > 100` so filter toggles stay snappy on huge reviews.
...
- **Command palette: arrow-key wrap-around** — carried (tick 33 backlog). At the list ends, ArrowDown from the last row wraps to the first (and ArrowUp from the first to the last); pairs with tick-32's scroll-into-view.
- **Repo detail: live timestamps + recent-reviews keyboard nav** — extend tick-33's repos-list treatment to the `/app/repos/[id]` detail page (last-review stat + the recent-reviews table still use static `formatRelative` and have no row nav).
- **Overview: recent-reviews live timestamps + row nav** — the overview's recent-reviews list still uses static `formatRelative` and has no row nav; bring it in line with the reviews list (LiveRelativeTime + optional `[data-review-row]` nav).
- **Empty-state CTA: extend to audit + budget + installations** — tick 33 wired EmptyStateActions into reviews/repos/sla; give the audit, budget, and installations empty states the same primary/secondary CTA treatment.
- **GlobalNav: visual "g" pending indicator** — a tiny corner toast ("g …") while the sequence window is armed, so the two-key nav has feedback (Linear shows the pending prefix). Auto-dismisses on the second key or timeout.
- **Findings page: severity-mix summary bar at top** — a compact SeverityRow (with hrefFor deep-links) above the findings list summarizing the *filtered* set, so the operator sees the mix without scrolling.
- **Sticky bar polish: subtle shadow on stuck state** — the new StickyBar has no visual cue that it's pinned vs inline; a faint bottom shadow once `position: sticky` engages would read better. Needs an IntersectionObserver sentinel since CSS can't style "is currently stuck".
