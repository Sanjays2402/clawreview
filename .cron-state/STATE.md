...
- ~~Audit page: sortable columns + filter chips~~ — DONE tick 31 (75e5fdb). Server-component rewrite driven by searchParams: sortable when/actor/action headers (aria-sort + arrow), active-filter chips (action/search/sort) with per-chip remove + clear-all, `useListKeyboardNav` over `[data-audit-row]`, GET-form search + action dropdown, restyled to dense lowercase/mono. Pure filterAuditEntries / sortAuditEntries helpers exported.
- ~~Findings page: severity legend as filter~~ — DONE tick 31 (8c8957a), shipped on the review-detail SeverityRow (the canonical severity breakdown). `hrefFor(sev)` opt makes each non-empty bar segment + legend entry a deep-link into `/app/reviews/:id/findings?severity=<sev>`, composing with the findings view's existing `?severity=` param. Zero-count severities stay inert+dimmed; prop omitted = purely presentational (overview chart unchanged).
- ~~Command palette: recent + dynamic review jump~~ — DONE tick 31 (5ce0231). Last 12 reviews as "jump: owner/repo #N" entries (status hint), fetched in the app layout + passed down. Grouped navigate / recent-reviews sections + subsequence fuzzy match over label/id/keywords (owner, repo, pr#, sha, status) with substring hits ranked first.
- **Review detail: agent timeline hover scrub** — ~~DONE tick 31 (b4e3a96)~~. AgentTimeline rows are hover/focus-interactive: active row brightens its bar + expands an inline readout (exact duration, share of total time, findings, status). Keyboard accessible (tabbable rows) with per-row aria-label. Uses the shared animate-fade-in language.
- ~~Theme toggle: system-preference default + tri-state~~ — DONE tick 31 (87027a8). ThemeToggle cycles light -> dark -> system; system mode follows OS prefers-color-scheme and reacts live to OS flips via matchMedia listener. Distinct monitor glyph for system mode (corner dot hints resolved palette). Boot script resolves the same 3-way model pre-paint (zero flash); legacy explicit choices preserved, missing key now = system. New apps/dashboard/src/lib/theme.ts centralises the model.
- **Reviews list: relative-time live refresh** — carried from tick 30. "3m ago" timestamps go stale on a left-open tab. A tiny client interval re-renders `formatRelative` outputs every 30s (intersection-gated so off-screen rows don't churn).

### Tick 31 — 2026-06-25 06:48 PT — 5 features (FRONTEND BATCH)

| # | Slice | SHA | Lines | Notes |
|---|---|---|---|---|
| 1 | Tri-state theme toggle (light -> dark -> system); system follows OS prefers-color-scheme live via matchMedia; pre-paint boot resolves same 3-way model (zero flash); new lib/theme.ts model module | 87027a8 | +110/-22 | theme-toggle rewrite, root layout ThemeBoot, new lib/theme.ts |
| 2 | SeverityRow `hrefFor(sev)` deep-links: bar segments + legend entries link into `/app/reviews/:id/findings?severity=<sev>`; zero-count inert+dimmed; omit prop = presentational | 8c8957a | +66/-9 | severity-row rewrite, review-detail wire |
| 3 | AgentTimeline hover/focus scrub: active row brightens bar + expands inline readout (duration, share-of-total, findings, status); tabbable + aria-labelled; animate-fade-in | b4e3a96 | +55/-13 | agent-timeline -> client component |
| 4 | Audit log server-component rewrite: sortable when/actor/action headers (aria-sort), active-filter chips (per-chip remove + clear-all), useListKeyboardNav, GET-form search, dense restyle | 75e5fdb | +245/-83 | audit-table rewrite, audit page searchParams |
| 5 | Command palette recent-review jump: last 12 reviews as grouped "jump: owner/repo #N" entries (status hint); subsequence fuzzy match over label/id/keywords with substring-first ranking | 5ce0231 | +130/-30 | command-palette rewrite, app-layout fetch+pass, shortcuts-overlay doc |

Gate results: dashboard `tsc --noEmit` output IDENTICAL to the tick-30 baseline — the ONLY line is the pre-existing TS5101 `baseUrl` deprecation on tsconfig.json:18; ZERO new type errors from this batch. Verified per-slice (each typechecked to the 5-line baseline as built) and once more after the final commit (5 lines). NOTABLE THIS TICK: disk was healthy (16Gi free on /, system NOT at 100%), and `next build` got materially further than tick 29/30 — it **compiled the production bundle successfully** ("Compiled successfully in 12.4s"; Google Fonts loaded fine this tick, unlike tick 30's CDN stall). The build then fails ONLY in Next's separate standalone type-check phase on `src/app/layout.tsx:5 import './globals.css'` (CSS side-effect module declaration not resolved by that phase) — proven pre-existing: the import line is byte-identical at parent commit 4d2acfa and was untouched by this batch (my ThemeBoot edit is lower in the file). So the production compile of all 5 slices is GREEN; the residual is an environmental Next build-typecheck quirk unrelated to this work. All changes scoped to `apps/dashboard/src/` (11 files, +630/-122); zero backend/packages touched. Push verified: `git fetch -q origin && git log --oneline origin/main | head -1` -> `5ce0231`.

**Tick-31 frontend override:** All five slices are user-facing UI work in `apps/dashboard/src/`. Each slice independently revertible, matching the existing lowercase / mono / dense / Linear-Raycast design language. 4 of the 10 tick-31 backlog items shipped + the agent-timeline-hover fresh item (5 total); 5 carried to tick 32. The parked backend roadmap (worker blame, dashboard-for-counters wiring, Grafana JSON, presets/filter-report hooks) stays parked under the override until Sanjay flips it back.

### Backlog seeded for tick 32 (refill — frontend-first under the standing override)
- **Reviews list: sticky filter bar on scroll** — carried. `sticky top-10` (under the 40px app header) + backdrop-blur on the reviews status-tab/chip strip and the findings filter strip so they stay reachable on long lists.
- **Reviews list: per-row hover preview** — carried. Hover a PR row ~400ms -> popover with the head finding mix (reuse SeverityMiniBar). Needs a thin `bySeverity` on ReviewListItem (keep minimal, wired straight to the row).
- **Findings page: virtualized list for 200+ findings** — carried. Windowed scroller (custom IntersectionObserver, no react-window dep) when `filtered.length > 100` so filter toggles stay snappy.
- **Reviews list: empty-state CTA improvements** — carried. Primary CTA "configure github app" -> /app/installations, secondary "view docs" -> /docs. Apply to repos + audit empty states too (audit's empty state was just restyled in tick 31; give it the CTA).
- ~~Reviews list: relative-time live refresh~~ — DONE tick 32 (c9070e2). New `LiveRelativeTime` client primitive re-formats `formatRelative` on a 30s interval, IntersectionObserver-gated (64px rootMargin) so off-screen rows don't churn; SSR-safe with `suppressHydrationWarning`; renders semantic `<time dateTime>`. Wired into the reviews-list `created` column AND the audit-log `when` column (kept inside the existing `[data-audit-row]` focus target).
- ~~Trends page: interactive chart polish~~ — DONE tick 32 (01b3ec0). Swapped the trends findings/day chart from static `Sparkline` to `InteractiveSparkline` (hover hit-targets, dot, dashed guide, edge-flipping readout, keyboard cursor) matching the overview. Extracted the duplicated `dayLabels(n)` into `lib/format.ts`; overview + trends now share it.
- ~~Command palette: keyboard scroll-into-view for active row~~ — DONE tick 32 (e83d068). Per-row callback-ref map keyed by flat index; `scrollIntoView({ block: 'nearest' })` on every idx change (and when the filtered set shifts under the cursor) so arrow-key nav keeps the highlighted row visible on the now-longer recent-review list.
- ~~SLA page: severity filter tabs + chips~~ — DONE tick 32 (289c044). New `SlaBreachesTable`: severity tabs with live count badges (`?sev=`), sortable severity/age/sla/overdue columns (aria-sort + arrow, severity by rank map), active-filter chips + clear-all, `useListKeyboardNav` over `[data-sla-row]`, dense lowercase/mono/sev restyle. Policy-override query keys threaded through every filter/sort link via a `policyParams` map.
- ~~Theme: respect prefers-reduced-motion~~ — DONE tick 32 (ff9e096). `globals.css` `@media (prefers-reduced-motion: reduce)` collapses every animation/transition to one near-instant frame (!important over Tailwind utils) + forces `scroll-behavior: auto`. JS-driven smooth scroll gated via new `lib/motion.ts` (`prefersReducedMotion()` / `motionScrollBehavior()`); finding deep-link scrollIntoView now jumps instantly under reduced motion.
- **Review detail: agent timeline keyboard scrub hint** — carried. tick-31 made the rows tabbable + hover-interactive; add a small "Tab to scrub" hint in the agents card header (mirroring the reviews-list j/k hint) so the new keyboard affordance is discoverable.

### Tick 32 — 2026-06-25 12:05 PT — 5 features (FRONTEND BATCH)

| # | Slice | SHA | Lines | Notes |
|---|---|---|---|---|
| 1 | Trends findings/day chart -> `InteractiveSparkline` hover-scrub (matches overview); extracted shared `dayLabels(n)` helper into lib/format.ts, removed overview's local copy | 01b3ec0 | +47/-22 | trends page, overview page, format.ts |
| 2 | SLA breaches: `SlaBreachesTable` with severity tabs (live counts) + sortable severity/age/sla/overdue columns (aria-sort, rank-map severity) + active-filter chips + clear-all + `[data-sla-row]` keyboard nav; policyParams threaded through all links | 289c044 | +332/-101 | new component, sla page rewrite |
| 3 | Command palette: per-row callback-ref map + `scrollIntoView({block:'nearest'})` on idx change so arrow-key nav keeps the active row visible on long fuzzy lists | e83d068 | +18/-3 | command-palette rewrite |
| 4 | `LiveRelativeTime` client primitive — 30s interval, IntersectionObserver-gated, SSR-safe + `suppressHydrationWarning`, semantic `<time>`; wired into reviews `created` + audit `when` columns | c9070e2 | +94/-4 | new component, reviews page, audit-table |
| 5 | `prefers-reduced-motion` accessibility gate — globals.css media block collapses animations/transitions + smooth scroll; new lib/motion.ts gates JS scrollIntoView; finding deep-link scroll respects it | ff9e096 | +49/-2 | globals.css, new lib/motion.ts, finding-row |

Gate results: dashboard `tsc --noEmit` output IDENTICAL to the tick-31 baseline — the ONLY line is the pre-existing TS5101 `baseUrl` deprecation on tsconfig.json:18; ZERO new type errors from this batch. Verified per-slice (each typechecked to the 1-line baseline as built) and once more after the final commit (`grep -c "error TS"` = 1). All changes scoped to `apps/dashboard/` (12 files, +536/-105); zero backend/packages touched (`git diff --name-only origin/main...HEAD | grep -v '^apps/dashboard/'` = empty). Disk healthy (16Gi free on /). Push verified: `git fetch -q origin && git log --oneline origin/main | head -1` -> ff9e096.

**Tick-32 frontend override:** All five slices are user-facing UI work in `apps/dashboard/src/`. Each slice independently revertible, matching the existing lowercase / mono / dense / Linear-Raycast design language. 5 of the 10 tick-32 backlog items shipped (relative-time live refresh, trends interactive chart, command-palette scroll-into-view, SLA tabs/chips/sort, prefers-reduced-motion); 5 carried to tick 33. The parked backend roadmap (worker blame, dashboard-for-counters wiring, Grafana JSON, presets/filter-report hooks) stays parked under the override until Sanjay flips it back.

### Backlog seeded for tick 33 (refill — frontend-first under the standing override)
- **Reviews list: sticky filter bar on scroll** — carried. `sticky top-10` (under the 40px app header) + backdrop-blur on the reviews status-tab/chip strip, the findings filter strip, and now the SLA severity-tab strip (tick 32) so they stay reachable on long lists.
- **Reviews list: per-row hover preview** — carried. Hover a PR row ~400ms -> popover with the head finding mix (reuse SeverityMiniBar). Needs a thin `bySeverity` on ReviewListItem (keep minimal, wired straight to the row).
- **Findings page: virtualized list for 200+ findings** — carried. Windowed scroller (custom IntersectionObserver, no react-window dep) when `filtered.length > 100` so filter toggles stay snappy.
- **Reviews list: empty-state CTA improvements** — carried. Primary CTA "configure github app" -> /app/installations, secondary "view docs" -> /docs. Apply to repos + audit + SLA empty states too for consistency.
- **Review detail: agent timeline keyboard scrub hint** — carried. Add a small "Tab to scrub" hint in the agents card header (mirroring the reviews-list j/k hint) so the new keyboard affordance is discoverable.
- **Repos list: relative-time live refresh** — extend tick-32's `LiveRelativeTime` to the repos `last review` column (currently a static `formatRelative`) so the third dense table with timestamps stays fresh too.
- **Trends: severity-mix deep-links** — apply tick-31's `SeverityRow hrefFor` treatment to the trends severity-mix card so each segment links to a filtered reviews/findings view, matching the review-detail interactivity.
- **Command palette: arrow-key wrap-around** — at the list ends, ArrowDown from the last row wraps to the first (and ArrowUp from the first to the last), a common palette affordance; pairs with tick-32's scroll-into-view.
- **SLA page: per-row keyboard-open hint + `bySeverity` Stat polish** — add the j/k/Enter hint to the SLA header (now that `[data-sla-row]` nav shipped) and consider a compact severity-mix bar in the stat row.
- **Findings filter strip: sticky on scroll** — the findings page filter strip (sev/state/agent/group) scrolls off on long file-grouped lists; `sticky` it under the breadcrumb like the planned reviews treatment.

### Tick 33 — 2026-06-25 17:11 PT — 5 features (FRONTEND BATCH)

| # | Slice | SHA | Lines | Notes |
|---|---|---|---|---|
| 1 | Global `g`-prefix go-to keyboard nav (`g o/r/p/i/t/s/b/a/c/k` -> the matching page); new `GlobalNav` listener mounted once in the app shell; 900ms sequence window; bare `g` never preventDefault-ed so list `g g` jump-to-first still works; ignores inputs + modifier chords; shortcuts overlay "go to" group + command-palette hints filled in | 5be3c9c | +110/-13 | new component, layout mount, overlay + palette doc |
| 2 | Agent timeline bottleneck "slowest" pill (only when 2+ agents AND >=40% of wall time) + Up/Down/Home/End arrow-key roving focus (per-row ref array) + "scrub" header hint; pill in aria-label | cd9b9f6 | +72/-7 | agent-timeline rewrite |
| 3 | `StickyBar` primitive (sticky top-10 under the 40px header, translucent bg + backdrop-blur, z-20) applied to reviews + repos + SLA tab strips and the findings filter strip so controls stay reachable on long lists | 5444f72 | +120/-49 | new primitive, 4 call sites |
| 4 | `EmptyStateActions` primitive (primary accent + secondary outlined CTA pair; internal Link / external anchor) wired filter-aware into reviews ("clear filters" vs "configure github app") + repos + SLA empty states | 2c30d5f | +110/-6 | new primitive, 3 empty states |
| 5 | Repos list parity: `ListKeyboardNav` over `[data-repo-row]` (j/k/gg/G + Enter, focus-visible ring + row tint + j/k header hint) + `LiveRelativeTime` on the "last review" column (30s interval, IO-gated) | d543981 | +90/-12 | repos page rewrite |

Gate results: dashboard `tsc --noEmit` output IDENTICAL to the tick-32 baseline — the ONLY line is the pre-existing TS5101 `baseUrl` deprecation on tsconfig.json:18; ZERO new type errors from this batch (`grep -c "error TS"` = 1, non-TS5101 = 0). Verified per-slice (each typechecked to the 1-line baseline as built) and once more after the final commit. NOTABLE THIS TICK: disk was healthy (14Gi free on /), and `next build` **compiled the production bundle successfully** ("Compiled successfully in 13.7s"). The build then fails ONLY in Next's separate standalone type-check phase on `src/app/layout.tsx:5 import './globals.css'` (CSS side-effect module declaration not resolved by that phase) — proven pre-existing AND untouched: I edited the app-GROUP layout (`src/app/app/layout.tsx`), NOT the root `src/app/layout.tsx`; the failing import line is byte-identical at origin/main and HEAD, and the root layout is absent from `git diff --name-only origin/main...HEAD`. So the production compile of all 5 slices is GREEN; the residual is the documented environmental Next build-typecheck quirk (identical to tick 31). All changes scoped to `apps/dashboard/src/` (11 files, +425/-81); zero backend/packages touched (`git diff --name-only origin/main...HEAD | grep -v '^apps/dashboard/'` = empty). Push verified: `git fetch -q origin && git log --oneline origin/main | head -1` -> `d543981`.

**Tick-33 frontend override:** All five slices are user-facing UI work in `apps/dashboard/src/`. Each slice independently revertible, matching the existing lowercase / mono / dense / Linear-Raycast design language. 1 of the 10 tick-33 backlog items shipped from the explicit list (sticky filter bars) + 4 fresh-but-backlog-adjacent items (global g-nav, agent-timeline bottleneck, empty-state CTAs, repos parity — all carried/listed themes). 4 explicit items carried to tick 34. The parked backend roadmap (worker blame, dashboard-for-counters wiring, Grafana JSON, presets/filter-report hooks) stays parked under the override until Sanjay flips it back.

### Backlog seeded for tick 34 (refill — frontend-first under the standing override)
- **Reviews list: per-row hover preview** — carried (tick 30+). Hover a PR row ~400ms -> popover with the head finding mix (reuse SeverityMiniBar). Needs a thin `bySeverity` on ReviewListItem (keep minimal, wired straight to the row).
- **Findings page: virtualized list for 200+ findings** — SKIPPED (deliberately, tick 34). Conflicts with the existing `[data-finding-row]` DOM-query keyboard nav AND the focus-scroll deep-linking (virtualization unmounts off-screen rows, breaking both). Not worth the quality regression; needs a rethink (keep rows mounted, window only the paint) before it's shippable.
- **Trends: severity-mix deep-links** — carried (tick 33 backlog). Apply tick-31's `SeverityRow hrefFor` treatment to the trends severity-mix card so each segment links to a filtered reviews/findings view, matching the review-detail interactivity.
- ~~Command palette: arrow-key wrap-around~~ — DONE tick 34 (04dfe76). ArrowDown past the last result wraps to the first (modular index), ArrowUp before the first wraps to the last; added Home/End jump-to-ends; footer + shortcuts overlay document the keys.
- **Repo detail: live timestamps + recent-reviews keyboard nav** — carried (tick 34 shipped the *overview* variant; the `/app/repos/[id]` detail page last-review stat + recent-reviews table still use static `formatRelative` and have no row nav).
- ~~Overview: recent-reviews live timestamps + row nav~~ — DONE tick 34 (7323714). Rows are `[data-review-row]` focus targets driven by shared ListKeyboardNav (j/k/gg/G + Enter); created column swaps to LiveRelativeTime; j/k header hint.
- **Empty-state CTA: extend to audit + budget + installations** — carried. tick 33 wired EmptyStateActions into reviews/repos/sla; give the audit, budget, and installations empty states the same primary/secondary CTA treatment.
- ~~GlobalNav: visual "g" pending indicator~~ — DONE tick 34 (5466a6c). "g then o r p t s ..." corner toast while the 900ms window is armed; auto-dismisses on the second key or a mirrored client-side timeout; pointer-events-none + aria-hidden.
- ~~Findings page: severity-mix summary bar at top~~ — DONE tick 34 (aff90e7), shipped counting over the state/agent scope but IGNORING the active severity filter so every segment stays a live deep-link (a one-click severity *switcher*, not just a readout); clicking the active severity toggles back to all.
- ~~Sticky bar polish: subtle shadow on stuck state~~ — DONE tick 34 (1b1324f). StickyBar is now a client component watching its own pin state via IntersectionObserver (root top shrunk to the stick line + 1px); raises a soft bottom shadow when pinned. Applies to all four call sites.

### Tick 34 — 2026-06-25 21:52 PT — 5 features (FRONTEND BATCH)

| # | Slice | SHA | Lines | Notes |
|---|---|---|---|---|
| 1 | StickyBar stuck-state shadow: client component watches its own pin via IntersectionObserver (rootMargin top shrunk to stick line + 1px; ratio < 1 -> pinned); raises `shadow-[0_6px_16px_-10px_...]` + `transition-shadow`. No sentinel sibling -> layout/space-y unchanged. Auto-applies to reviews + repos + sla + findings strips | 1b1324f | +56/-7 | sticky-bar.tsx -> client |
| 2 | Command palette arrow-key wrap-around (`(i+1)%len` / `(i-1+len)%len`) + Home/End jump-to-ends; empty-list guard; footer hint + shortcuts-overlay palette group updated | 04dfe76 | +24/-7 | command-palette + shortcuts-overlay |
| 3 | GlobalNav pending-prefix toast: "g then o r p t s ..." bottom-left toast while the 900ms sequence window is armed; mirrored client timer auto-dismisses; pointer-events-none + aria-hidden + animate-fade-in | 5466a6c | +50/-6 | global-nav.tsx (null -> conditional toast) |
| 4 | Overview recent-reviews parity: `[data-review-row]` focus targets + shared ListKeyboardNav (j/k/gg/G + Enter) + LiveRelativeTime on created column + j/k header hint; drops unused formatRelative import | 7323714 | +24/-13 | overview page |
| 5 | Findings severity-mix summary bar: compact SeverityRow over the state/agent scope (ignores active severity filter so every segment stays a live deep-link); hrefWith composes filters; active-severity click toggles back to all; zero-scope guard | aff90e7 | +41/0 | findings page |

Gate results: dashboard `tsc --noEmit` output IDENTICAL to the tick-33 baseline — the ONLY line is the pre-existing TS5101 `baseUrl` deprecation on tsconfig.json:18; ZERO new type errors from this batch (`grep "error TS" | grep -v TS5101` = empty; total tsc output = 5 lines, same as baseline). Verified per-slice (each typechecked clean to the 5-line baseline as built) and once more after the final commit. NOTABLE THIS TICK: disk healthy (15Gi free on /, 45% used), and `next build` **compiled the production bundle successfully** ("Compiled successfully in 11.2s"). The build then fails ONLY in Next's separate standalone type-check phase on `src/app/layout.tsx:5 import './globals.css'` (CSS side-effect module declaration not resolved by that phase) — proven pre-existing AND untouched: the failing import line is byte-identical at origin/main and HEAD (`git show origin/main:.../layout.tsx | sed -n '5p'` == `git show HEAD:...`), and the root layout is absent from `git diff --name-only origin/main...HEAD`. So the production compile of all 5 slices is GREEN; the residual is the documented environmental Next build-typecheck quirk (identical to ticks 31/33). All changes scoped to `apps/dashboard/src/` (6 files, +162/-23); zero backend/packages touched (`git diff --name-only origin/main...HEAD | grep -v '^apps/dashboard/'` = empty). Push verified: `git fetch -q origin && git log --oneline origin/main | head -1` -> `aff90e7`.

**Tick-34 frontend override:** All five slices are user-facing UI work in `apps/dashboard/src/`. Each slice independently revertible, matching the existing lowercase / mono / dense / Linear-Raycast design language. 4 of the 10 tick-34 backlog items shipped (command-palette wrap, overview parity, global-nav toast, findings severity-mix, sticky shadow) + the virtualized-findings item was deliberately SKIPPED (conflicts with keyboard nav + deep-link focus scroll; documented above). 5 carried/remaining to tick 35. The parked backend roadmap (worker blame, dashboard-for-counters wiring, Grafana JSON, presets/filter-report hooks) stays parked under the override until Sanjay flips it back.

### Backlog seeded for tick 35 (refill — frontend-first under the standing override)
- **Reviews list: per-row hover preview** — carried (tick 30+). Hover a PR row ~400ms -> popover with the head finding mix (reuse SeverityMiniBar). Needs a thin `bySeverity` on ReviewListItem (keep minimal, wired straight to the row).
- **Trends: severity-mix deep-links** — carried (tick 33+). Apply tick-31's `SeverityRow hrefFor` treatment to the trends severity-mix card so each segment links to a filtered reviews/findings view. (Note: trends severity is account-wide, not per-review — the link target should be `/app/reviews` filtered, or a findings search, not a single review's findings view. Resolve that mapping when building.)
- **Repo detail: live timestamps + recent-reviews keyboard nav** — carried. Extend tick-34's overview treatment to `/app/repos/[id]`: last-review stat + recent-reviews table to LiveRelativeTime + `[data-review-row]` ListKeyboardNav + j/k hint, and dense-restyle the recent-reviews table to match the reviews list.
- **Empty-state CTA: extend to audit + budget + installations** — carried. Give the audit, budget (installations + repo-health), and installations empty states the EmptyStateActions primary/secondary CTA treatment.
- **Findings page: windowed paint for 200+ findings (rethink of the skipped virtualization item)** — keep all rows MOUNTED (so `[data-finding-row]` nav + focus-scroll deep-linking keep working) but defer the expensive expanded-content paint of off-screen rows via IntersectionObserver, so filter toggles stay snappy without removing rows from the DOM. The naive react-window-style approach was rejected tick 34 for breaking nav; this is the constraint-aware version.
- **Agent timeline: keyboard scrub hint discoverability** — the tick-31/33 agent-timeline rows are tabbable + arrow-navigable, but the "Tab to scrub" hint only shows in the card header; consider a focus-ring + first-row auto-hint so the affordance is discovered on first Tab into the card.
- **Command palette: section-jump (Tab cycles navigate <-> recent reviews)** — pairs with tick-34's wrap-around; Tab/Shift-Tab could jump between the two sections rather than stepping row-by-row on a long list.
- **Reviews/repos/sla: stuck-bar shadow now exists — add a "scroll to top" affordance** — once the sticky bar is pinned (tick-34 shadow proves we can detect it), surface a small "back to top" control in the pinned bar for very long lists.
- **Theme toggle: transition flash audit** — verify the tri-state theme toggle (tick 31) + prefers-reduced-motion (tick 32) interplay has no color-flash on system->explicit switches; add a short crossfade if it reads abruptly.
- **Findings deep-link: copy-link toast confirmation** — tick-30's copy-deep-link button shows an inline check; consider a brief corner toast (like the new global-nav toast) so the copy confirmation is visible even if the button scrolled off.



### Tick 35 — 2026-06-26 02:15 PT — 5 features (FRONTEND BATCH)

| # | Slice | SHA | Lines | Notes |
|---|---|---|---|---|
| 1 | Repo detail (/app/repos/[id]) dense restyle: last-review stat + recent-reviews `created` column -> LiveRelativeTime; recent-reviews table is `[data-review-row]` + shared ListKeyboardNav (j/k/gg/G + Enter) + j/k header hint; lowercase/mono restyle of stat cards + pause/resume forms + table; empty state -> EmptyStateActions | 7484793 | +228/-117 | repo detail page rewrite |
| 2 | Installations page dense rewrite: PageHeader replaces 2xl heading; `[data-install-row]` ListKeyboardNav + j/k/open hint; type column gains Buildings/User glyph; spent cell gains 24px budget-utilization bar (emerald/medium/critical); formatUsd; empty state -> EmptyStateActions (install on github -> /login) | 6c54d90 | +110/-46 | installations page rewrite |
| 3 | Budget repo-health rows clickable -> /app/repos/<owner>__<repo>; `[data-health-row]` ListKeyboardNav + j/k/open hint; last-review -> LiveRelativeTime; both empty states gain EmptyStateActions; focus-visible ring + hover tint | 3e51000 | +66/-15 | budget page |
| 4 | StickyBar opt-in `backToTop` control: reuses the tick-34 `stuck` signal to fade in a right-aligned jump-to-top button ONLY while pinned; window.scrollTo honors motionScrollBehavior() (prefers-reduced-motion); wired into reviews + repos + SLA tab strips | 3f5e022 | +46/-9 | sticky-bar primitive + 3 call sites |
| 5 | Command palette Tab/Shift-Tab section-jump: Tab -> first row of next section, Shift-Tab -> previous (modular wrap); always preventDefault so focus never escapes the modal; footer hint + shortcuts-overlay palette group document the keys | 4ab5f8c | +33/-3 | command-palette + shortcuts-overlay |

Gate results: dashboard `tsc --noEmit` output IDENTICAL to the tick-34 baseline — the ONLY line is the pre-existing TS5101 `baseUrl` deprecation on tsconfig.json:18; ZERO new type errors from this batch (`grep -c "error TS"` = 1, non-TS5101 = 0). Verified per-slice (each typechecked to the 1-line baseline as built) and once more after the final commit. NOTABLE THIS TICK: disk healthy (14Gi free on /, 46% used), and `next build` **compiled the production bundle successfully** ("Compiled successfully in 8.1s"). The build then fails ONLY in Next's separate standalone type-check phase on `src/app/layout.tsx:5 import './globals.css'` (CSS side-effect module declaration not resolved by that phase) — proven pre-existing AND untouched: the failing import line is byte-identical at origin/main and HEAD (`git show origin/main:.../layout.tsx | sed -n '5p'` == `git show HEAD:...`), and the root layout is absent from `git diff --name-only origin/main...HEAD`. So the production compile of all 5 slices is GREEN; the residual is the documented environmental Next build-typecheck quirk (identical to ticks 31/33/34). All changes scoped to `apps/dashboard/src/` (9 files, +333/-133); zero backend/packages touched (`git diff --name-only origin/main...HEAD | grep -v '^apps/dashboard/'` = empty). Push verified: `git fetch -q origin && git log --oneline origin/main | head -1` -> `4ab5f8c`.

**Tick-35 frontend override:** All five slices are user-facing UI work in `apps/dashboard/src/`. Each slice independently revertible, matching the existing lowercase / mono / dense / Linear-Raycast design language. 2 of the explicit tick-35 backlog items shipped (repo detail live-time+nav, command-palette section-jump) + 3 backlog-adjacent items (installations rewrite, budget rows clickable, sticky-bar back-to-top — all from carried themes). The parked backend roadmap (worker blame, dashboard-for-counters wiring, Grafana JSON, presets/filter-report hooks) stays parked under the override until Sanjay flips it back.

### Backlog seeded for tick 36 (refill — frontend-first under the standing override)
- **Reviews list: per-row hover preview** — carried (tick 30+). Hover a PR row ~400ms -> popover with the head finding mix (reuse SeverityRow/mini-bar). Needs a thin `bySeverity` on ReviewListItem (server DTO add; keep minimal, wired straight to the row).
- **Trends: severity-mix deep-links** — carried (tick 33+). Apply tick-31's `SeverityRow hrefFor` treatment to the trends severity-mix card. NOTE: trends severity is account-wide, not per-review — link target should be `/app/reviews` filtered (or a findings search), not a single review's findings view. Resolve that mapping when building.
- **Findings page: windowed paint for 200+ findings** — carried/reworked (the naive virtualization was rejected tick 34 for breaking `[data-finding-row]` nav + focus-scroll). Constraint-aware version: keep all rows MOUNTED, defer only the expensive expanded-content paint of off-screen rows via IntersectionObserver.
- ~~Empty-state CTA: extend to audit + api-keys + integrations + team~~ — DONE tick 36 (54794f0). EmptyStateActions wired into audit-log (no entries), integrations (no llm providers), team + api-keys placeholders (with real copy). config left as-is (it has a live ConfigValidator, not an empty state).
- ~~Agent timeline: first-Tab discoverability~~ — DONE tick 36 (b31313a). Tracks DOM focus per row (focusedIdx, distinct from hover-active); on first focus surfaces an inline "↑↓ move between agents" hint, suppressed permanently once the user presses an arrow (scrubbed flag); adds a focus-visible inset ring on the focused row.
- **Reviews/repos/sla: "back to top" now exists — keyboard `gg` parity** — the new StickyBar back-to-top is mouse-only; the lists already have `gg` (jump to first row) via ListKeyboardNav, but a bare-page `gg` (scroll the window to top when no row is focused) would complete the affordance.
- **Theme toggle: transition flash audit** — verify the tri-state theme toggle (tick 31) + prefers-reduced-motion (tick 32) interplay has no color-flash on system->explicit switches; add a short crossfade if it reads abruptly.
- ~~Findings deep-link: copy-link toast confirmation~~ — DONE tick 36 (520fd69). New global toast bus + corner Toaster mounted in the app shell (window CustomEvent `cr:toast`); finding-row copy button fires `toast('deep link copied')` alongside the inline check so the confirmation is visible even when the row scrolled off.
- ~~Repo detail: agent-timeline-style spend/findings sparkline~~ — DONE tick 36 (cc8509a). InteractiveSparkline of findings-per-review (oldest->newest, PR-number labels) between the pause controls and recent-reviews table; header shows avg/peak/count; only renders with 2+ reviews.
- ~~Installations detail (settings/billing): dense restyle~~ — DONE tick 36 (c5f97af) for the BILLING page (Breadcrumbs + dense PageHeader + back-to-budget + severity-palette usage bar + "N% used" readout). The `/settings` page restyle remains TODO (carried to tick 37).

### Tick 36 — 2026-06-26 07:33 PT — 5 features (FRONTEND BATCH)

| # | Slice | SHA | Lines | Notes |
|---|---|---|---|---|
| 1 | Empty-state CTA pair on the four remaining dead-end empty states: audit-log (no entries -> view reviews / docs), integrations (no llm providers -> configure / view config), team + api-keys placeholders rewritten with real copy + CTAs. config skipped (live ConfigValidator, not an empty state) | 54794f0 | +57/-6 | 4 files (team, api-keys, integrations, audit-table) |
| 2 | Repo detail (/app/repos/[id]) findings-per-review InteractiveSparkline between pause controls + recent-reviews table; oldest->newest, PR-number hover labels; avg/peak/count header + oldest<->newest axis caption; renders only with 2+ reviews | cc8509a | +52/0 | repo detail page |
| 3 | Agent-timeline first-Tab scrub discoverability: per-row DOM focus tracking (focusedIdx, distinct from hover-active) -> inline "↑↓ move between agents" hint on first focus, suppressed permanently after the first arrow press (scrubbed); focus-visible inset ring on the focused row | b31313a | +44/-5 | agent-timeline |
| 4 | Global toast bus + corner Toaster (window CustomEvent `cr:toast`, stacks 3 self-expiring, mono/blur/fade, aria-live) mounted in the app shell; finding-row copy-deep-link fires `toast('deep link copied')` alongside the inline check so the confirmation survives the row scrolling off | 520fd69 | +89/0 | new toaster.tsx + layout + finding-row |
| 5 | Installation billing page (/app/installations/[id]/billing) dense restyle: Breadcrumbs + dense PageHeader + back-to-budget + severity-palette usage bar + live "N% used" header readout; lowercase/mono throughout, zero behavior change | c5f97af | +84/-58 | billing page rewrite |

Gate results: dashboard `tsc --noEmit` output IDENTICAL to the tick-35 baseline — the ONLY line is the pre-existing TS5101 `baseUrl` deprecation on tsconfig.json:18; ZERO new type errors from this batch (`grep "error TS" | grep -v TS5101 | wc -l` = 0). Verified per-slice (each typechecked clean to the baseline as built) and once more after the final commit. NOTABLE THIS TICK: disk healthy (14Gi free on /, 46% used), and `next build` **compiled the production bundle successfully** ("Compiled successfully in 9.1s"). The build then fails ONLY in Next's separate standalone type-check phase on `src/app/layout.tsx:5 import './globals.css'` (CSS side-effect module declaration not resolved by that phase) — proven pre-existing AND untouched: the failing import line is byte-identical at origin/main and HEAD (`git show origin/main:.../layout.tsx | sed -n '5p'` == `git show HEAD:...`), and the root layout is absent from `git diff --name-only origin/main...HEAD` (I edited the app-GROUP layout `src/app/app/layout.tsx`, NOT the root `src/app/layout.tsx`). So the production compile of all 5 slices is GREEN; the residual is the documented environmental Next build-typecheck quirk (identical to ticks 31/33/34/35). All changes scoped to `apps/dashboard/src/` (10 files); zero backend/packages touched (`git diff --name-only origin/main...HEAD | grep -v '^apps/dashboard/'` = empty). Push verified: `git fetch -q origin && git log --oneline origin/main | head -1` -> `c5f97af`.

**Tick-36 frontend override:** All five slices are user-facing UI work in `apps/dashboard/src/`. Each slice independently revertible, matching the existing lowercase / mono / dense / Linear-Raycast design language. 5 of the explicit tick-36 backlog items shipped (empty-state CTAs, repo-detail sparkline, agent-timeline first-Tab hint, copy-link toast, billing dense restyle). The parked backend roadmap (worker blame, dashboard-for-counters wiring, Grafana JSON, presets/filter-report hooks) stays parked under the override until Sanjay flips it back.

### Backlog seeded for tick 37 (refill — frontend-first under the standing override)
- **Reviews list: per-row hover preview** — carried (tick 30+). Hover a PR row ~400ms -> popover with the head finding mix (reuse SeverityRow/mini-bar). Needs a thin `bySeverity` on ReviewListItem (server DTO add; keep minimal, wired straight to the row).
- **Trends: severity-mix deep-links** — carried (tick 33+). Apply tick-31's `SeverityRow hrefFor` treatment to the trends severity-mix card. NOTE: trends severity is account-wide -> link target should be `/app/reviews` filtered (or a findings search), not a single review's findings view.
- **Findings page: windowed paint for 200+ findings** — carried/reworked. Keep all rows MOUNTED (so `[data-finding-row]` nav + focus-scroll keep working), defer only the expensive expanded-content paint of off-screen rows via IntersectionObserver.
- **Installation settings page: dense restyle** — carried from tick 36 (billing shipped, settings remains). The `/app/installations/[id]/settings` page still uses text-xl headings + slate table styling; bring it in line with the dense lowercase/mono language + a `[data-repo-row]` nav on the repos table + LiveRelativeTime on last-review.
- **Reviews/repos/sla: bare-page `gg` scroll-to-top parity** — the StickyBar back-to-top is mouse-only; a bare-page `gg` (scroll window to top when no row is focused) would complete the keyboard affordance against the existing ListKeyboardNav `gg` (jump to first row).
- **Theme toggle: transition flash audit** — verify the tri-state theme toggle (tick 31) + prefers-reduced-motion (tick 32) interplay has no color-flash on system->explicit switches; add a short crossfade if it reads abruptly.
- **Toast adoption: pause/resume + dismiss/reopen confirmations** — the new tick-36 Toaster is wired only to copy-link; the repo pause/resume forms and finding dismiss/reopen actions are server actions whose success is currently silent. Fire a toast on each so the operator gets feedback (needs a client wrapper since they're `<form action>` server actions).
- **Agent timeline: bottleneck "share" inline bar** — the slowest-agent pill calls out the bottleneck, but the share-of-total is only in the scrub readout; a faint share track under the duration bar would make the proportion glanceable without focusing each row.
- **Repo detail: spend-per-review sparkline toggle** — tick-36 added a findings sparkline; a small toggle to switch the same chart between findings and spend (totalCostUsd) would reuse the InteractiveSparkline for a second metric without a second card.
- **Command palette: recent-review status glyph** — the palette's "jump: owner/repo #N" rows show the status as plain text in the hint slot; a small colored StatusPill-style dot would make running/failed reviews scannable in the list.

### Tick 37 — 2026-06-26 13:23 PT — 5 features (FRONTEND BATCH)

| # | Slice | SHA | Lines | Notes |
|---|---|---|---|---|
| 1 | Installation settings page (/app/installations/[id]/settings) dense restyle: PageHeader + lowercase Breadcrumbs; stat cards -> mono uppercase labels + tabular-nums; budget card gains severity-palette utilization bar + "N% used"; repos table is `[data-repo-row]` + shared ListKeyboardNav (j/k/gg/G + Enter) + j/k hint; last-review -> LiveRelativeTime; EmptyStateActions CTA pair. ALSO fixes a latent broken repo link (`owner%2Frepo` percent-encoding -> `owner__repo` slug that parseSlug actually parses; old links 404'd) | 079e22c | +200/-103 | settings page rewrite |
| 2 | Toast confirmation on repo pause/resume: extract the two bare `<form action={serverAction}>` forms into a `RepoPauseControls` client component that runs the action in a useTransition + fires a corner toast on success ("reviews paused/resumed on owner/repo"); pending label + disable while in flight (prevents double-fire). Server actions passed in as props -- thin feedback shell, no logic moves client-side | 58e2068 | +99/-39 | new component + page wire |
| 3 | Agent-timeline share-of-total inline track: a second thinner (3px) fainter bar under each duration bar, width = agent's fraction of total wall time (tracks sum to ~100%), so "where does the time go?" is glanceable across all rows without hovering. Bottleneck track tinted severity-high; active rows brighten to accent; title attr surfaces exact % | 5ecd212 | +35/-9 | agent-timeline |
| 4 | Repo-detail findings<->spend sparkline toggle: new `formatValue` hook on InteractiveSparkline (spend renders `$0.42` not `0.42 finding`); new `RepoTrendCard` client component holds the toggle + swaps which server-computed series the single sparkline renders (avg/peak reformats per metric); second metric costs zero extra card. Both series from the existing listReviews call | 600ce57 | +130/-44 | new card + sparkline hook |
| 5 | Command-palette recent-review status dot: small filled dot before each "jump: owner/repo #N" label, colored off StatusPill's tone ladder (emerald completed / critical failed / accent running / medium queued / ...) so review state is scannable in the palette like every list. Route rows carry no status -> no dot (navigate section unchanged); aria-hidden | 58ad77b | +28/-2 | command-palette |

Gate results: dashboard `tsc --noEmit` output IDENTICAL to the tick-36 baseline — the ONLY line is the pre-existing TS5101 `baseUrl` deprecation on tsconfig.json:18; ZERO new type errors from this batch (`grep "error TS" | grep -v TS5101 | wc -l` = 0; total tsc output = 1 line, same as baseline). Verified per-slice (each typechecked clean to the 1-line baseline as built) and once more after the final commit. NOTABLE THIS TICK: disk healthy (13Gi free on /, 48% used), and `next build` **compiled the production bundle successfully** ("Compiled successfully in 14.0s"). The build then fails ONLY in Next's separate standalone type-check phase on `src/app/layout.tsx:5 import './globals.css'` (CSS side-effect module declaration not resolved by that phase) — proven pre-existing AND untouched: the root layout is absent from `git diff --name-only origin/main...HEAD` (this batch touched NO layout files at all), and the failing import line is byte-identical at origin/main and HEAD (`git show origin/main:.../layout.tsx | sed -n '5p'` == `git show HEAD:...`). So the production compile of all 5 slices is GREEN; the residual is the documented environmental Next build-typecheck quirk (identical to ticks 31/33/34/35/36). All changes scoped to `apps/dashboard/src/` (7 files, +406/-154); zero backend/packages touched (`git diff --name-only origin/main...HEAD | grep -v '^apps/dashboard/src/'` = empty). Push verified: `git fetch -q origin && git log --oneline origin/main | head -1` -> `58ad77b`.

**Tick-37 frontend override:** All five slices are user-facing UI work in `apps/dashboard/src/`. Each slice independently revertible, matching the existing lowercase / mono / dense / Linear-Raycast design language. 5 of the explicit tick-37 backlog items shipped (installation settings dense restyle, pause/resume toast, agent-timeline share track, repo-detail spend toggle, command-palette status dot). The parked backend roadmap (worker blame, dashboard-for-counters wiring, Grafana JSON, presets/filter-report hooks) stays parked under the override until Sanjay flips it back.

### Backlog seeded for tick 38 (refill — frontend-first under the standing override)
- **Reviews list: per-row hover preview** — carried (tick 30+). Hover a PR row ~400ms -> popover with the head finding mix (reuse SeverityRow/mini-bar). Needs a thin `bySeverity` on ReviewListItem (server DTO add; keep minimal, wired straight to the row).
- **Trends: severity-mix deep-links** — carried (tick 33+). Apply tick-31's `SeverityRow hrefFor` treatment to the trends severity-mix card. NOTE: trends severity is account-wide -> link target should be `/app/reviews` filtered (or a findings search), not a single review's findings view. (SeverityRow already supports `hrefFor`; the work is choosing the right account-wide target.)
- **Findings page: windowed paint for 200+ findings** — carried/reworked. Keep all rows MOUNTED (so `[data-finding-row]` nav + focus-scroll keep working), defer only the expensive expanded-content paint of off-screen rows via IntersectionObserver.
- **Reviews/repos/sla: bare-page `gg` scroll-to-top parity** — carried. The StickyBar back-to-top is mouse-only; a bare-page `gg` (scroll window to top when no row is focused) would complete the keyboard affordance against the existing ListKeyboardNav `gg` (jump to first row).
- **Theme toggle: transition flash audit** — carried. Verify the tri-state theme toggle (tick 31) + prefers-reduced-motion (tick 32) interplay has no color-flash on system->explicit switches; add a short crossfade if it reads abruptly.
- **Toast adoption: finding dismiss/reopen confirmations** — tick 37 wired pause/resume toasts; the finding-row dismiss/reopen actions (in finding-row.tsx, already a client component with useTransition) are still silent on success. Fire a toast ("finding dismissed" / "finding reopened") so bulk triage gives feedback even when the row collapses/scrolls.
- **Bulk findings bar: toast on bulk dismiss** — the BulkFindingsBar applies a filter-scoped dismiss to N findings; surface a toast ("dismissed N findings") on completion, matching the new single-finding feedback.
- **Repo detail: spend trend severity tint** — the new findings<->spend toggle (tick 37) draws spend in the same accent color as findings; tint the spend series (or its peak marker) when a review's spend is an outlier vs the repo's avg, so cost spikes pop without reading the hover.
- **Installations list: budget utilization bar parity** — tick 35 dense-restyled the installations LIST with a 24px budget bar; tick 37 gave the SETTINGS page a severity-palette bar with "N% used". Reconcile the two so the list bar uses the same emerald/medium/critical tone ladder + readout.
- **Command palette: group count badges** — ~~the palette section headers ("navigate", "recent reviews") could show a small count of matches in each group~~ — DONE tick 38 (8711ad7). Per-section tabular count on the right of each header; hidden for single-row sections; aria-hidden.

### Tick 38 — 2026-06-26 18:35 PT — 5 features (FRONTEND BATCH)

| # | Slice | SHA | Lines | Notes |
|---|---|---|---|---|
| 1 | Toast feedback on finding dismiss/reopen + bulk triage: finding-row fires `toast('finding dismissed'/'reopened')`; bulk-findings-bar fires `toast('dismissed N findings')` using the server-reported `updated` count. Reuses tick-36 Toaster bus; inline errors unchanged; survives the row/bar scrolling off during a fast x/r pass | ac216c9 | +24/-2 | finding-row, bulk-findings-bar |
| 2 | Command palette per-section match-count badges: small tabular count on the right of each section header (`navigate` / `recent reviews`) so a fuzzy query narrowing to "2 reviews vs 8 routes" is legible at a glance; hidden for single-row sections; aria-hidden | 8711ad7 | +12/-2 | command-palette |
| 3 | Installations list budget-bar parity with the settings page: same tone ladder (spend>=budget critical, >=80% medium, else emerald) + a "{n}% used" / "no budget set" readout under the 24px bar; rounds utilization the same way the settings page does so an account reads consistently in the list and on its detail page | d8b5221 | +14/-4 | installations list |
| 4 | Theme-switch crossfade (flash audit): `.theme-transition` class on <html> for ~220ms around a palette flip fades only color-bearing props (bg/fg/border/accent/fill/stroke); self-removes + coalesces rapid toggles; wired into applyThemeMode AND the live OS day/night flip; gated behind `prefers-reduced-motion: no-preference` so reduced-motion users keep the instant cut; pre-paint boot script untouched (initial loads stay flash-free) | abc8701 | +63/-2 | globals.css, lib/theme.ts, theme-toggle |
| 5 | Trends agent-performance table -> dense duration bars: restyled the last flat slate `text-sm` table to lowercase/mono; sorted slowest-first with a "slowest" pill (2+ agents); proportional avg-duration bar normalised to the slowest, tinted by error rate (emerald clean / medium some / critical >=25%) so "slow AND flaky" is one glance; error-rate column reads critical when non-zero | 6a2d24c | +59/-23 | trends page |

Gate results: dashboard `tsc --noEmit` output IDENTICAL to the tick-37 baseline — the ONLY line is the pre-existing TS5101 `baseUrl` deprecation on tsconfig.json:18; ZERO new type errors from this batch (`grep 'error TS' | grep -v TS5101` = empty; total = 1 line, same as baseline). Verified per-slice (each typechecked to the 1-line baseline as built) and once more after the final commit. NOTABLE THIS TICK: disk healthy (13Gi free on /, 49% used), and `next build` **compiled the production bundle successfully** ("Compiled successfully in 11.5s"). The build then fails ONLY in Next's separate standalone type-check phase on `src/app/layout.tsx:5 import './globals.css'` (CSS side-effect module declaration not resolved by that phase) — proven pre-existing AND untouched: the root layout is absent from `git diff --name-only origin/main...HEAD` (this batch touched globals.css CONTENT but not the import line or the root layout), and the failing import line is byte-identical at origin/main and HEAD (`git show origin/main:.../layout.tsx | sed -n '5p'` == `git show HEAD:...` == `import './globals.css';`). So the production compile of all 5 slices is GREEN; the residual is the documented environmental Next build-typecheck quirk (identical to ticks 31/33/34/35/36/37). All changes scoped to `apps/dashboard/src/` (8 files, +160/-23); zero backend/packages touched (`git diff --name-only origin/main...HEAD | grep -v '^apps/dashboard/src/'` = empty). Push verified: `git fetch -q origin && git log --oneline origin/main | head -1` -> `6a2d24c`.

**Tick-38 frontend override:** All five slices are user-facing UI work in `apps/dashboard/src/`. Each slice independently revertible, matching the existing lowercase / mono / dense / Linear-Raycast design language. 4 of the explicit tick-38 backlog items shipped (finding dismiss/reopen toasts, bulk-dismiss toast, command-palette group count badges, installations budget-bar parity) + 2 carried-theme items (theme-toggle flash audit, trends agent-performance bars). The parked backend roadmap (worker blame, dashboard-for-counters wiring, Grafana JSON, presets/filter-report hooks) stays parked under the override until Sanjay flips it back.

### Backlog seeded for tick 39 (refill — frontend-first under the standing override)
- **Reviews list: per-row hover preview** — carried (tick 30+). Hover a PR row ~400ms -> popover with the head finding mix (reuse SeverityRow/mini-bar). Needs a thin `bySeverity` on ReviewListItem (server DTO add; keep minimal, wired straight to the row). This is the longest-carried item; worth either shipping the DTO add or formally retiring it.
- **Trends: severity-mix deep-links** — carried (tick 33+). Apply tick-31's `SeverityRow hrefFor` to the trends severity-mix card. NOTE: trends severity is account-wide -> link target should be `/app/reviews` filtered (or a findings search), not a single review's findings view. (SeverityRow already supports `hrefFor`; the work is choosing the right account-wide target.)
- **Findings page: windowed paint for 200+ findings** — carried/reworked. Keep all rows MOUNTED (so `[data-finding-row]` nav + focus-scroll keep working), defer only the expensive expanded-content paint of off-screen rows via IntersectionObserver.
- **Reviews/repos/sla: bare-page `gg` scroll-to-top parity** — carried. The StickyBar back-to-top is mouse-only; a bare-page `gg` (scroll window to top when no row is focused) would complete the keyboard affordance against the existing ListKeyboardNav `gg` (jump to first row).
- **Toast adoption: rerun + budget-update confirmations** — tick 37/38 wired pause/resume + dismiss/reopen + bulk toasts. The remaining silent server actions are the review rerun-form (`rerun-form.tsx`) and the budget-update form (`budget-form.tsx`). Fire a toast on each so every mutating action gives consistent feedback.
- **Repo detail: spend trend severity tint** — carried (tick 37+). The findings<->spend toggle draws spend in the same accent color; tint the spend series (or its peak marker) when a review's spend is an outlier vs the repo's avg, so cost spikes pop without reading the hover.
- **Agent timeline / trends: shared bottleneck idiom extraction** — both AgentTimeline (per-review) and now the trends agent table (tick 38) compute "slowest agent, normalise to max, tint by status/error". Consider a tiny shared `AgentDurationBar` primitive so the two stay visually in lock-step as either evolves.
- **Command palette: scroll the active section header into view** — tick-32 added per-row scroll-into-view; when Tab section-jumps (tick 35) to a section whose header is above the viewport, the header label isn't pinned. A sticky section header inside the scroll container would keep the group label visible.
- **SLA page: severity-mix summary bar** — the SLA breaches table has severity tabs (tick 32) but no at-a-glance mix bar like the findings page got (tick 34). Add a compact SeverityRow over the breach set, each segment deep-linking to its `?sev=` tab.
- **Empty-state CTA: settings + config dead-ends** — tick 36 wired CTAs into audit/integrations/team/api-keys. The `/app/settings` page and the config page's non-validator empty states are the last text-only dead-ends; give them the EmptyStateActions treatment for consistency.
