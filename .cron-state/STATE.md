...
- **Worker `findingDigest({ blame: ... })` server-side wiring** — carried.
- **Telemetry `clawreview_review_drift_watch_polls_total` Prometheus exposition through the server** — carried.
- **CLI `stats --jsonl --no-footer`** — carried.
- **CLI `review filter-report --watch --on-applied-change <cmd>` hook** — pair the tick-24 watch loop with a hook that fires on the false->true (or true->false) transition of the `applied` bit. Use case: "alert me when the config rollout finally landed on this review's worker run" (or vice versa: "alert me if the worker started ignoring the filter"). Same shape as tick-16 `review drift --watch --on-drift` so an operator who knows one hook knows the other.
- **Server `/api/reviews/:id/filter-report ?fields` deny-list (-prefix)** — mirror of /digest's ?slim minus-prefix form. Lets a consumer say "give me everything EXCEPT appliedFilters" without enumerating the rest. Mutex with the allowlist form (can't mix +foo and -bar).
- **CLI `review filter-report --watch --on-applied-template slack|webhook`** — paired with the above hook flag, mirrors --on-drift-template / --on-recover-template / tick-24's --on-regression-template. Closed slack|webhook set, env-var fallback ladder.
- **Telemetry `clawreview_review_filter_report_read_duration_seconds` Grafana dashboard JSON** — once the histogram is landed (tick 24) the next natural artifact is a pre-baked Grafana panel JSON committed to docs/dashboards/ so a fresh deploy can import the slim-vs-full latency view without recreating it by hand.
- **CLI `review filter-report --diff <baseReviewId>`** — two-review compare for the filter-report endpoint (analogous to `review drift --base/--target`). Fetches both /filter-report bodies and surfaces the delta (which fields changed; which thresholds drifted). Use case: "did this CI run land with the new min_confidence threshold compared to the previous green build?".
