...
- **Aggregator `findingDigest` topTags worker rewire** — surface topTags inside the PR-comment header (the tick-15 slice lands the data; the comment renderer is the natural consumer now that worker + dashboard + CLI all read it via findingDigest).
- **CLI `clawreview review drift --watch --on-drift <cmd>` hook** — extension of tick-15 watch. When a poll surfaces drift, exec `<cmd>` with the drift report on stdin. For on-calls who want to fire a Slack alert / open a dashboard when drift first appears, without writing a shell wrapper around tail-f.
- **CLI `clawreview presets diff --since-range <a>..<b>` range syntax** — sugar that splits `<a>..<b>` into `--since-base <a> --since-target <b>`. Mirrors git's `git log a..b` shape so an operator who reaches for it expects the same syntax to work.
- **Server `/api/reviews/:id/digest` `?include=topTags` projection knob** — opt-in narrowing for the new tick-15 topTags slice. Today every /digest response carries the full sparse `byTag` map; some dashboards just want the top-N slice and would rather skip the wire payload for an empty `(untagged)` count.
- **Telemetry `observeReviewDigestPersistedDrift` log-line on `stale`** — fires today via `log.info` in the worker. Add a corresponding structured `level: warn` line when a re-run flips a previously-`unchanged` review to `stale` (drift appeared between runs), so an on-call's existing log alerting picks it up without needing the Prometheus pipeline.
