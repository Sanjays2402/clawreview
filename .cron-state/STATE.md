...
- **Worker fires `clawreview_review_digest_drift_total{kind}` on review completion** — today the metric only fires on the read-side `/digest` endpoint. Adding a write-side fire (when the worker re-runs and persisted differs from fresh) closes the observability gap.
- **CLI `clawreview review drift --watch <reviewId>` (poll mode)** — natural follow-up to tick-14's `review drift` command. Today it consumes one body; `--watch` polls the server every N seconds and re-renders the drift banner. For on-calls watching a bulk-dismiss roll out.
- **Aggregator `findingDigest` top-tags slice** — mirror of topAgents / topCategories / topFiles, capped at opts.topTags (default 10, ceiling 200). Sparse maps are great for math but a dashboard panel wants the pre-sorted top-N slice.
- **CLI `clawreview presets diff --since-base <ref> --since-target <ref>` (independent refs per chain)** — extension of tick-14's `--since`. Today --since resolves chain A at the ref and chain B at HEAD; the natural symmetric case is "chain B at one ref, chain A at another". Useful for backporting comparisons.
- **Server `/api/reviews/:id/digest` `?recompute=fresh|cached` switch** — today the route always recomputes; a `?recompute=cached` mode returns the persisted digest only (no fresh recompute, no drift) for dashboards that already pulled the full /api/reviews/:id body and just want the persisted shape without re-walking.
