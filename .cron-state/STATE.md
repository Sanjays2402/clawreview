...
- **CLI `review filter-report --diff --on-delta <cmd>` hook** — fire a command when computeFilterReportDelta surfaces a non-empty delta. Mirrors `presets diff --on-drift`'s pattern for CI workflows. Closed `--on-delta-template slack|webhook` template form pairs with it (mirrors tick-25's --on-applied-template). New WatchOnDriftExecer reuse for the executor seam.
- **CLI `review filter-report --diff --json-stream`** — line-delimited JSON stream variant of the diff output (mirrors tick-23's `stats --jsonl`). Header line carries baseId/targetId; one line per delta axis (applied / inputTotal / droppedTotal / minConfidence / severityThreshold); footer line with hasDelta + summary. Use case: log aggregator pipelines that ingest one JSON event per line.
- **Server `/api/reviews/:id/filter-report ?fields=*` star sugar** — mirror of /digest's tick-19 ?slim=* shorthand. `?fields=*` resolves to "all fields" (back-compat with absent ?fields, but explicit); `?fields=*` mixed with named fields rejects (standalone-only). Case-insensitive.
- **CLI `review filter-report --diff --output <path>`** — write the JSON delta to a file (mirrors `presets diff --output` from tick 12). `--output -` writes to stdout. Composes with text-mode reject (artifact paths are JSON-only). New writeFilterReportDiffOutput helper.
- **Telemetry `clawreview_review_filter_report_diff_duration_seconds{result}` histogram** — per-invocation latency for the diff command, labelled by the same result label as the counter (identical | delta | error). Pairs with the tick-25 counter so a dashboard can compute average latency per result class.
