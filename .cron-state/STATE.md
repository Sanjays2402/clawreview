...
- **Server `/api/internal/webhook/stats` `?bucketWindow=` end-time override** — carried from tick 11. Today the sparkline is "from now, walking back"; postmortem snapshots want "walking back from a specific incident time".
- **Aggregator `findingDigest` worker hand-off to comment header** — tick 11 wired the comment renderer to ACCEPT a `digest` opt; the worker still lets the renderer build its own. A follow-up should compute the digest ONCE per review, hand the same reference to the renderer AND to the dashboard's review-store summary, so a single tree walk powers both.
- **Operator-poll counter labels `?probe=name` from the bypass arm** — tick 11 bumps `result=bypass` against the probe label. A follow-up should also emit `clawreview_operator_poll_bypass_total{probe,reason}` to attribute WHY a bypass happened (force=1 today, future: hash-tag bypass, internal-network bypass) so a security review can audit the bypass surface separately from the volume metric.
- **CLI `clawreview presets diff --exclude-fields <a,b,c>` (mirror of --only-fields)** — for an operator who wants "everything EXCEPT this set". Pairs naturally with --only-fields (mutually exclusive). Same helper plumbing.
- **CLI `clawreview presets diff --output <path>` (write JSON / YAML to a file)** — for a migration ticket flow where the diff body needs to land on disk for a follow-up commit. Today the operator pipes stdout; an explicit `--output` is friendlier in CI and avoids shell quoting woes.
