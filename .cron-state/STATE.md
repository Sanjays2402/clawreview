...
- **Reviews list: relative-time live refresh** — carried. Tiny client interval re-renders `formatRelative` every 30s, intersection-gated so off-screen rows don't churn. Reuse for the new audit `when` column too (it now uses formatRelative).
- **Trends page: interactive chart polish** — apply the tick-30 InteractiveSparkline hover-scrub treatment (or BarChart hover readouts) to /app/trends so the trends data-viz matches the overview's interactivity.
- **Command palette: keyboard scroll-into-view for active row** — the palette's active index can scroll out of view on a long fuzzy-filtered list; add `scrollIntoView({ block: 'nearest' })` on idx change so arrow-key nav keeps the highlighted row visible. Natural follow-up to tick-31's recent-review entries (which lengthen the list).
- **SLA page: severity filter tabs + chips** — extend the tick-29/30/31 table-interactivity pattern to /app/sla breaches (status/severity tabs + chips + sortable columns) so every dense table behaves identically.
- **Theme: respect prefers-reduced-motion** — gate the animate-fade-in / transition utilities behind `motion-safe:` (or a reduced-motion media check) so the tooltip / sparkline / agent-scrub / palette animations honor the OS accessibility setting. Pairs naturally with tick-31's system-theme work.
- **Review detail: agent timeline keyboard scrub hint** — tick-31 made the rows tabbable + hover-interactive; add a small "Tab to scrub" hint in the agents card header (mirroring the reviews-list j/k hint) so the new keyboard affordance is discoverable.
