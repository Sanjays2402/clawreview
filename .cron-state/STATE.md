...
- **Worker `findingDigest({ blame: ... })` server-side wiring** — carried.
- **Telemetry `clawreview_review_drift_watch_polls_total` Prometheus exposition through the server** — carried.
- **Server `/api/reviews/:id/filter-report` Prometheus histogram** — pair the tick-23 read counter with a `clawreview_review_filter_report_read_duration_seconds{shape}` histogram so dashboards can see "slim responses are 3x faster than full" (or refute the assumption). The route already knows the shape; adding a histogram is a single observe() call.
- **CLI `review filter-report --watch <reviewId>` poll mode** — mirror of the `review drift --watch` pattern for the filter-report endpoint. Use case: an on-call watching a config rollout wants to see "did the new min_confidence threshold land on this review's worker run yet?" without rerunning curl every N seconds.
- **CLI `review filter-report --require-filter` gating flag** — exit 3 when the persisted report's `applied` bit is false. Pairs with the CLI's existing exit-3-on-drift contract for CI gates. Today the command always exits 0 on a successful fetch; the gating arm is intentionally reserved for the followup.
- **CLI `review drift --base --target --on-regression-template slack|webhook`** — mirror of tick-17's --on-drift-template for the compare command's --on-regression hook. ENV-var fallback ladder primary->shared so a single-channel operator doesn't have to set two env vars.
- **CLI `stats --jsonl --no-footer`** — opt-out of the footer line for a consumer that only wants the header + per-severity stream. Use case: a log aggregator that pre-baked schema for the bucket lines doesn't want a one-off footer mucking up its index. Default ON for back-compat.
- **Server `/api/reviews/:id/filter-report ?fields=appliedFilters,inputTotal` allowlist** — generalisation of ?slim=true to a comma-separated field allowlist (mirrors /digest's ?slim=byTag,byFile). Lets a dashboard panel that only needs `droppedTotal` ship the smallest possible payload.
