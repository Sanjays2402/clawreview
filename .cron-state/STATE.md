...
- **Worker `findingDigest({ blame: ... })` server-side wiring** — carried.
- **Telemetry `clawreview_review_drift_watch_polls_total` Prometheus exposition through the server** — carried.
- **CLI `clawreview review drift --base <reviewId> --target <reviewId>` (compare two reviews)** — carried from tick 21. Take two review IDs and compute the digest drift between them ("did the bug fix actually clear the findings?"). Pairs with the tick-20 filter flags so an operator can ask "did the bug fix clear high-confidence high-severity findings?".
- **CLI `stats --filter-summary --json-header` (machine-readable filter summary on stdout-pre-JSON)** — natural follow-up to tick-21's text header. For pipelines that want a one-line JSON header before the report body so a downstream tool can short-circuit without parsing the full JSON.
- **Server `/api/reviews/:id/digest ?filterDropEcho=true` opt** — surfaces `filterDropped: number` on the response alongside `minConfidence` / `severityThreshold` so a dashboard can render "filtered 12 of 20" without re-walking findings. The droppedTotal already exists on the new findingDigestWithFilterReport; just exposing it on the wire.
- **Aggregator `findingDigestWithFilterReport` worker wiring** — pair the tick-21 helper with the worker's review-completion path so the persisted digest can ALSO carry `droppedTotal` / `appliedFilters` metadata for legacy reviews and future cached recompute drift detection. Today the worker uses bare `findingDigest`; switching to the wrapper is a no-op on shape but unlocks future "did this review actually apply a filter at write time?" telemetry.
- **Telemetry: `clawreview_findings_filter_pre_applied_total{phase}` worker-side counter** — close-set `{aggregate, worker_post}` so the dashboard can distinguish "filter ran at aggregate() time" from "filter ran in the post-aggregate findingDigest pass". Pairs with the worker wiring above.
