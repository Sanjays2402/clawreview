...
- **CLI `clawreview review drift --watch --on-drift-cmd-template`** — sugar on top of tick-16's `--on-drift`. A predeclared template like `slack` would expand to a curl command with the SLACK_WEBHOOK env var so on-calls don't have to wrap their own pipeline.
- **Server `/api/reviews/:id/digest` `?slim` accepts a comma-separated field list** — `?slim=byTag,byAgent` so a consumer can strip JUST the heaviest map without losing the others. Today it's all-or-nothing (`true`/`false`).
- **Aggregator `findingDigest` `topAuthors` slice** — fourth top-N slice alongside topTags/topAgents/topCategories. Today the dashboard's Top Contributors block re-walks the blame map; this helper would pre-rank it in one pass like the others.
- **CLI `presets diff --since-range <a>...<b>`** — symmetric-difference variant of the tick-16 range sugar. Pairs with git's `a...b` syntax (merge-base resolution); useful for "what changed on this branch independently of the other?".
- **Telemetry `clawreview_review_drift_watch_polls_total{result}` counter** — instrumentation for the tick-15 watch loop. Closed `{ok, drift, timeout, error}` label set; pairs with the existing drift counters for a full picture of how the dashboard / on-call consumes /digest.
