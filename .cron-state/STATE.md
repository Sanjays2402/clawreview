...
- **CLI `clawreview presets diff --since <ref>` (compute chain a from a git ref)** — carried from tick 13 refill. For "what changed in `.clawreview/presets/<name>.yml` between two commits?" — a thin wrapper that resolves both presets via git show then runs the existing diff.
- **CLI `clawreview review drift <reviewId>`** — surface `computeDigestDrift(record.digest, findingDigest(record.findings))` from the server's review-store endpoint so an operator can spot stale review headers without opening the dashboard. Pairs with tick-13's `computeDigestDrift` and tick-12's persisted digest.
- **Worker drift-detection metric — `clawreview_review_digest_drift_total{kind}`** — closed set `['stale', 'fresh']` so Prom can graph the rate of drift events across all reviews. Fires once per `/api/reviews/:id` recompute (rate-limited so it doesn't trip on every dashboard refresh).
- **Server `/api/reviews/:id/digest` lightweight DTO** — returns `{ persisted, fresh, drift }` for the dashboard's drift banner without fetching the full findings payload (today the dashboard pulls every finding just to recompute). Pairs with tick-13's `computeDigestDrift` helper.
- **Aggregator `findingDigest` per-tag bucket (`byTag`)** — extend the digest with one more sparse bucket keyed by finding.tags (each finding contributes to N buckets). Dashboard / CLI gains "findings by tag" without re-walking findings.
- **CLI `clawreview presets diff --output -` size-check assertion** — when the body exceeds a configurable threshold (default 100KB), refuse to write and exit 2 with a hint to switch to `--output <path>`. Prevents a pipeline from accidentally piping a multi-megabyte diff into a downstream tool.
