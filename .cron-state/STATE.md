...
- **Empty-state CTA: extend to audit + api-keys + integrations + team** — tick 31/33/35 wired EmptyStateActions into reviews/repos/sla/installations/budget/repo-detail empty states. Give the remaining empty states (audit, api-keys, integrations, team, config) the same primary/secondary CTA treatment for consistency.
- **Agent timeline: first-Tab discoverability** — the agent-timeline rows are tabbable + arrow-navigable, but the "↑↓ scrub" hint only shows in the card header; consider a focus-ring + first-row auto-hint so the affordance is discovered on first Tab into the card.
...
- **Theme toggle: transition flash audit** — verify the tri-state theme toggle (tick 31) + prefers-reduced-motion (tick 32) interplay has no color-flash on system->explicit switches; add a short crossfade if it reads abruptly.
- **Findings deep-link: copy-link toast confirmation** — carried. tick-30's copy-deep-link button shows an inline check; a brief corner toast (like the global-nav toast) so the copy confirmation is visible even if the button scrolled off.
- **Repo detail: agent-timeline-style spend/findings sparkline** — the repo detail page (rewritten tick 35) shows a flat recent-reviews table; a small findings-per-review or spend sparkline above it would match the overview/trends interactive-sparkline language.
- **Installations detail (settings/billing): dense restyle** — the installations LIST is now dense (tick 35); the `/app/installations/[id]/settings` + `/billing` detail pages may still use the older heading/table styling. Audit + bring them in line.
